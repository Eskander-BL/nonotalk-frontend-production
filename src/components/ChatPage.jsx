...
export default function ChatPage() {
  ...
  const playedAudiosRef = useRef(new Set())
  ...
  // Jouer l'audio des messages AI quand audio_path passe de null à non-null
  useEffect(() => {
    messages.slice(-10).forEach((message) => {
      if (!message.is_user && message.audio_path && !playedAudiosRef.current.has(message.audio_path)) {
        playedAudiosRef.current.add(message.audio_path)
        setShowThinking(false)
        playAudio(message.audio_path).catch(() => {})
      } else if (!message.is_user && !message.audio_path) {
