...
export function useVoice() {
  ...
  const startRecording = useCallback(async (onTranscriptionComplete) => {
    ...
    try {
      ...
      // MAIS sur mobile (iOS/Android), on préfère MediaRecorder + backend pour plus de fiabilité
      const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent)
      const SpeechRec = typeof window !== 'undefined' && (window.SpeechRecognition || window.webkitSpeechRecognition)
      if (SpeechRec && !isMobile) {
      ...
      const SILENCE_DURATION = 800 // ~0.8s de silence (coupure plus réactive pour mobile)
      ...
      // Timeout de sécurité : arrêt automatique après 10s (important pour mobile)
